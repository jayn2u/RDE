...
def load_yaml_config(
    ...
    *,
    ...
    if config["encode_variant"] not in {"bge", "tse", "bge+tse"}:
        raise ValueError(f"encode_variant must be one of: bge, tse, bge+tse in {config_path}.")
...
def output_model_tag(checkpoint_dir: str, encode_variant: str) -> str:
    base = Path(checkpoint_dir).as_posix().replace("/", "-").replace(":", "-")
    return f"{base}_{encode_variant}"
...
def _encode_image_batch(model, images: torch.Tensor, encode_variant: str) -> torch.Tensor:
    if encode_variant == "tse":
        return model.encode_image_tse(images)
    feat = model.encode_image(images)
    if encode_variant == "bge+tse":
        tse_feat = model.encode_image_tse(images)
        return F.normalize(F.normalize(feat, dim=-1) + F.normalize(tse_feat, dim=-1), dim=-1)
    return feat


def _encode_text_batch(model, text_tokens: torch.Tensor, encode_variant: str) -> torch.Tensor:
    if encode_variant == "tse":
        return model.encode_text_tse(text_tokens)
    feat = model.encode_text(text_tokens)
    if encode_variant == "bge+tse":
        tse_feat = model.encode_text_tse(text_tokens)
        return F.normalize(F.normalize(feat, dim=-1) + F.normalize(tse_feat, dim=-1), dim=-1)
    return feat
    ...
    for batch in tqdm(loader, desc="encode probe images", dynamic_ncols=True):
        ...
        with torch.amp.autocast(device_type=device.type, enabled=use_amp):
            batch_features = F.normalize(
                _encode_image_batch(model, images, encode_variant), dim=-1
            ).cpu()
        if features.numel() == 0:
...
def encode_unique_texts(
    ...
    *,
    ...
    for batch in tqdm(loader, desc="encode probe texts", dynamic_ncols=True):
        ...
        with torch.amp.autocast(device_type=device.type, enabled=use_amp):
            batch_features = F.normalize(
                _encode_text_batch(model, text_tokens, encode_variant), dim=-1
            ).cpu()
        if features.numel() == 0:
...
def main() -> None:
    config_path = require_config_path("SUGARCREPE_CONFIG", "sugarcrepe")
    config = load_yaml_config(
        config_path,
        defaults=CONFIG_DEFAULTS,
        required=("dataset", "checkpoint_dir"),
    )
    ...
    train_config_path = checkpoint_dir / "configs.yaml"
    ...
    image_features_tensor = encode_unique_images(
        ...
        device=device,
        use_amp=use_amp,
        encode_variant=config.encode_variant,
    )
    ...
    model_tag = output_model_tag(str(checkpoint_dir), config.encode_variant)
    output = {
        "benchmark": "sugarcrepe",
        "model": "RDE",
        "dataset": dataset_name,
