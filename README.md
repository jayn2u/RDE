...
`run_rde.sh` enables W&B tracking. Before the first run, create the repository
environment file:

```bash
cp env/.env.example env/.env
# Fill in WANDB_API_KEY and WANDB_ENTITY. WANDB_PROJECT defaults to rde.
```

Process environment variables take precedence over `env/.env`. To run
...
primary `val/t2i_*` and `val/i2t_*` values use the final BGE+TSE score; component
results are available under `val/bge_*` and `val/tse_*`. Each output directory
also contains `wandb_meta.json` and `wandb_run_id`.
