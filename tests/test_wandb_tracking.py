...
class WandbTrackingTest(unittest.TestCase):
    ...
    def test_default_env_file_is_repository_env_dotenv(self):
        ...
        with tempfile.TemporaryDirectory() as project_root:
            ...
            with open(env_path, "w", encoding="utf-8") as env_file:
                env_file.write(
                    ...
                    "WANDB_ENTITY=root-env-entity\n"
                    "WANDB_PROJECT=root-env-project\n"
                )
            ...
            with (
                ...
                patch("utils.wandb_tracking.PROJECT_ROOT", project_root),
        ...
        self.assertEqual(
            fake_wandb.init_kwargs["project"],
            "root-env-project",
        )
        self.assertEqual(
            fake_wandb.init_kwargs["entity"],
            "root-env-entity",
        )
    ...
    def test_dataset_root_defaults_from_repository_env_dotenv(self):
        with tempfile.TemporaryDirectory() as project_root:
            ...
            with (
                patch.dict(os.environ, {}, clear=True),
                patch("utils.options.PROJECT_ROOT", project_root, create=True),
                patch.object(sys, "argv", ["train.py"]),
            ):
                args = get_args()

        self.assertEqual(args.root_dir, "/tmp/rde-test-datasets")
